...
| 层 | 技术 | 角色 |
|---|------|------|
| **Agent 编排** | LangGraph StateGraph + `send()` fan-out + MemorySaver Checkpointer | 核心引擎 |
...
每个 Agent 的降级路径因职责不同而异，下表列出各自的实际退化次序：
...
| SWOTSynthesizer | _try_llm_synthesize() | _derived_swot()（基于画像）→ _fallback_swot()（模板）|
| ReportWriter | _try_llm_write() | _derived_report()（基于画像）→ _fallback_report()（模板）|
| Reviewer | _try_llm_review() | _fallback_review()（规则评分）|

这意味着系统在**没有 API Key 的情况下也能给出有意义的结果**——所有 Agent 在 LLM 不可用时自动降级到规则推导或模板输出，不会因网络故障而崩溃。
...
| 阶段 | 目标 | 状态 |
|------|------|------|
| **Week 1** | 项目骨架 + 7 Agent Pipeline + 意图解析 + 审计追踪 + 测试 + API | ✅ |
| **Week 2** | LangGraph 迁移（StateGraph + `send()` 并行子图 + Checkpointer）+ 真实 Search/Scrape/RAG Worker | ✅ |
| **Week 3** | 4 Agent LLM 接入（Market/SWOT/Writer/Reviewer）+ 三层降级 + 报告格式化增强 | ✅ |
| **Week 4** | 独立前端 UI + 依赖声明 + WebSocket 回放说明 + README 完善 | ✅ |
